...
impl Walrus {
    ...
    pub fn read_next_with_position(&self, col_name: &str, checkpoint: bool) -> io::Result<Option<(Entry, WalPosition)>> {
            ...
            if let Ok(idx_guard) = self.read_offset_index.read() {
                if let Some(pos) = idx_guard.get(col_name) {
                    if (pos.cur_block_idx & TAIL_FLAG) != 0 {
                        ...
                        // Mirror the persisted tail into in-memory state so subsequent
                        // `read_next` calls (which skip hydration) still see it via the
                        // `info.tail_*` fallback in the tail-path else-branch below.
        ...
        // If we have a persisted tail and some sealed blocks were recovered, fold into the
        // last block. Only clear `persisted_tail` if we actually folded — otherwise the
        // tail-path code below needs to honour it. Without this guard a persisted tail
        // pointing at the still-active block (chain empty) gets silently reset to
        // offset 0, which is wrong when callers explicitly set the cursor via
        ...
        loop {
            ...
            if let Some((tail_block_id, tail_off)) = persisted_tail {
                ...
                // No persisted_tail in this call (typically: hydration already ran on a
                // prior read). Fall back to the in-memory tail recorded by either a prior
                // read or by hydration mirroring — both cases mean we've already claimed
