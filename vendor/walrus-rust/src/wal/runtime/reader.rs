...
use crate::wal::{block::Block, config::debug_print};
...
impl Reader {
    ...
    pub(super) fn append_block_to_chain(&self, col: &str, block: Block) -> io::Result<()> {
        ...
        if let Some(info_arc) = {
            let map = self.data.read().map_err(|_| io::Error::new(io::ErrorKind::Other, "reader map read lock poisoned"))?;
            map.get(col).cloned()
        } {
        ...
        // slow path
        let info_arc = {
            let mut map = self.data.write().map_err(|_| io::Error::new(io::ErrorKind::Other, "reader map write lock poisoned"))?;
        ...
        debug_print!("[reader] chain append(slow/new): col={}, block_id={}, chain_len {}->{}", col, block.id, 0, 1);
        Ok(())
    }
}
