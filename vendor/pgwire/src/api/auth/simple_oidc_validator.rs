...
impl SimpleOidcValidator {
    ...
    pub async fn new(issuer: impl Into<String>) -> Result<Self, PgWireError> {
        ...
        Ok(Self { issuer, client, key_cache: Arc::new(RwLock::new(HashMap::new())), jwks_uri: Arc::new(RwLock::new(Some(uri))) })
    }
    ...
    fn jwk_to_pem(&self, jwk: &Jwk) -> Result<String, PgWireError> {
        if jwk.kty != "RSA" {
            return Err(PgWireError::OAuthValidationError(format!("only RSA key type is supported. Got: {}", jwk.kty)));
        }

        let n = jwk.n.as_ref().ok_or_else(|| PgWireError::OAuthValidationError("modulus is missing".to_string()))?;
        let e = jwk.e.as_ref().ok_or_else(|| PgWireError::OAuthValidationError("exponent is missing".to_string()))?;

        let n_bytes = BASE64_URL_SAFE_NO_PAD.decode(n).map_err(|e| PgWireError::OAuthValidationError(e.to_string()))?;
        let e_bytes = BASE64_URL_SAFE_NO_PAD.decode(e).map_err(|e| PgWireError::OAuthValidationError(e.to_string()))?;
        ...
        public_key.to_public_key_pem(LineEnding::LF).map_err(|e| PgWireError::OAuthValidationError(e.to_string()))
    }
    ...
    async fn get_pk(&self, kid: &str) -> Result<DecodingKey, PgWireError> {
        // check if it is present in the cache first
        {
            ...
            if let Some(pem) = cache.get(kid) {
                return DecodingKey::from_rsa_pem(pem.as_bytes()).map_err(|err| PgWireError::OAuthValidationError(err.to_string()));
            }
        }
        ...
        let jwks: Jwks = self
            ...
            .send()
            .await
            .map_err(|err| PgWireError::OAuthValidationError(format!("failed to fetch jwks from uri: {uri}. Err: {}", err)))?
            .json()
            .await
            .map_err(|err| PgWireError::OAuthValidationError(format!("invalid jwks format. Err {}", err)))?;

        let jwk: Jwk = jwks.keys.into_iter().find(|k| k.kid == kid).ok_or_else(|| PgWireError::OAuthValidationError(format!("key not found: {}", kid)))?;
        ...
        DecodingKey::from_rsa_pem(pem.as_bytes()).map_err(|err| PgWireError::OAuthValidationError(err.to_string()))
    }
}

#[async_trait]
impl OauthValidator for SimpleOidcValidator {
    async fn validate(&self, token: &str, username: &str, _issuer: &str, required_scopes: &str) -> PgWireResult<ValidatorModuleResult> {
        let header = decode_header(token).map_err(|e| PgWireError::OAuthValidationError(format!("Invalid token header: {}", e)))?;

        let kid = header.kid.ok_or_else(|| PgWireError::OAuthValidationError("Missing 'kid' in token".to_string()))?;
        ...
        let token_data =
            decode::<Claims>(token, &key, &validation).map_err(|e| PgWireError::OAuthValidationError(format!("Token validation failed: {}", e)))?;

        let claims = token_data.claims;

        // the OIDC says to prefer email or usernmae over sub
        let authn_id = claims.preferred_username.or(claims.email).unwrap_or(claims.sub.clone());

        // confirm username
        if username != authn_id && username != claims.sub {
            return Ok(ValidatorModuleResult { authorized: false, authn_id: Some(authn_id), metadata: None });
        }
        ...
        Ok(ValidatorModuleResult { authorized: true, authn_id: Some(authn_id), metadata: None })
    }
}
