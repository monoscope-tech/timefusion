    ...
    fn sql_statement_to_plan_with_context_impl(
        ...
        match statement {
            ...
            Statement::Update(Update {
                ...
                // TimeFusion patch (apache/datafusion#19950): the upstream
                // defensive guard rejected `UPDATE ... FROM` before reaching
                // `update_to_plan`, even though `update_to_plan` already
                // threads `from` through `plan_from_tables` to build the join.
                // Removing the guard lets `UPDATE ... FROM` flow through
                // DataFusion as `Dml { input: Projection→Filter→Join }`, which
                // TimeFusion's `DmlQueryPlanner` lowers to `MergeBuilder` on
                // the Delta side and a hash-join in MemBuffer.
