...
fn main() {
    ...
    // TIMEFUSION PATCH (only delta vs crates.io tikv-jemalloc-sys 0.6.1): let the
    // caller pick the profiler's backtrace method. Upstream passes `--enable-prof`
    // alone, which leaves jemalloc on the libgcc unwinder; in our release binary
    // that unwinder returns zero frames, so every heap dump collapsed into a single
    // `prof_backtrace_impl` frame (2026-07-31: 100% of an 88GB dump, unattributable).
    // `JEMALLOC_SYS_PROF_BACKTRACE=libunwind|gcc|libgcc` selects the method.
    if env::var("CARGO_FEATURE_PROFILING").is_ok() {
        info!("CARGO_FEATURE_PROFILING set");
        cmd.arg("--enable-prof");
        let method = read_and_watch_env("JEMALLOC_SYS_PROF_BACKTRACE").ok().map(|m| m.trim().to_owned()).filter(|m| !m.is_empty());
        if let Some(method) = method {
            assert!(
                ["libunwind", "gcc", "libgcc"].contains(&method.as_str()),
                "JEMALLOC_SYS_PROF_BACKTRACE must be libunwind, gcc or libgcc (got {method})"
            );
            info!("--enable-prof-{method}");
            cmd.arg(format!("--enable-prof-{method}"));
            if method == "libunwind" {
                // configure links jemalloc against libunwind but the crate never
                // forwards its LIBS to rustc; the final binary needs it too.
