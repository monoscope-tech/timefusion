...
PATHSET_rs='src tests benches build.rs Cargo.toml Cargo.lock rust-toolchain.toml rustfmt.toml .cargo proto schemas vendor .config/nextest.toml'
...
PATHSET_fmt='src tests benches build.rs rustfmt.toml rust-toolchain.toml'
...
detect_caps() {
  ...
  command -v protoc >/dev/null 2>&1 && caps="$caps protoc"
...
cmd_selftest() {
  ...
  # proto/ is in @rs but deliberately not in @fmt — rustfmt never reads it.
  WORKTREE_TREE=''
  : > "proto/$probe"; WORKTREE_TREE=''; after=$(fingerprint fmt); rm -f "proto/$probe"
