...
from bench.query_routing_matrix import Run, ambient_miss_reasons, load_manifest, planned_route, result_digest, stats_delta, stop_reason, validate_sql, window_bounds
...
class QueryRoutingMatrixTest(unittest.TestCase):
    ...
    def test_stats_delta_and_route_keep_miss_reason(self):
        ...
        self.assertEqual(planned_route(["DataSourceExec table=otel_logs_and_spans"]), "raw")
        self.assertEqual(planned_route(["DataSourceExec table=otel_logs_and_spans_rollup_dashboard_1h_v2"]), "full")
        self.assertEqual(
            planned_route(["otel_logs_and_spans", "otel_logs_and_spans_rollup_dashboard_1h_v2"]),
            "hybrid",
        )
    ...
    def test_stop_conditions_are_bounded(self):
        self.assertEqual(stop_reason(Run("q", "7d", "sealed", "first", "", "", planned_route="unknown"), 100), "long_window_route_unproven")
        self.assertEqual(
