...
## FINAL VERDICT (2026-09-12): write-capture earns its keep — THE GATE WAS DELETED

**Outcome: `TIMEFUSION_WRITE_CAPTURE_L2` no longer exists.** The measurement
settled the question, so the switch it fed became unreachable code and went with
it (`cd09d85f`), along with the per-key hit-before-evict tracking that answered
it (`56212cad`) — a `DashMap` probe on every cache hit is not worth paying to
keep re-answering something closed.

What survives is the byte-per-source accounting, because the **616 MB/s is still
unattributed** and attributing admissions is exactly what the next hypothesis
will need.
...
round trip. Per this plan's own decision table that is the **"do not flip"** branch — and
since the flag would then never be flipped, it was deleted rather than left as a
switch nobody would reach for during an incident.
...
The byte counters stay — they turned a plausible story into a measured refusal. That is the whole
