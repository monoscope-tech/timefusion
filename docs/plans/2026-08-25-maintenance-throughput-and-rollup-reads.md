...
|---|---|---|---|
| 25 | ✅ **DONE 2026-08-28** — repair converged, `DAMAGED_CELLS` emptied | Cursor reads **81/81** for both sources (unobservable until now — it has no `timefusion_stats` key; read via `docker cp` of the journal). All **81 base cells match a raw control**; `28f62f01`/08-24 converged 1,036,140 → 3,837,269 = raw, before sealing. Const emptied + made a config parameter in `5d696741`, not deleted — the 1h repair needs the same machinery under a **v3** key | — |
| 33 | 🔴 **NEW P0** — the 1h DERIVED tier is wrong on **29 of those same 81**, both directions | Only visible once the base became a trustworthy reference. +70.6% / +43.6% and down to −99.8% (one cell has 1 of 24 hour slots). **Not lag** (all 29 `derived_rollup` COMPLETE, none pending) and **not stale** (two written *today*, after `43ee5d84`). Wide-window dashboards read this tier. **Not rebuilt on purpose**: the code that produced them ran today, so a rebuild republishes them | fixing the derived path — start at the 08-25 "derived RETRIES rather than publishing short" guard, and the MoR-version summing for the over-counts. See `docs/plans/2026-08-28-damage-repair-converged-and-the-1h-tier.md` |
...
| 15 | ✅ **DECIDED: no spec change, ever** | The measures are FAITHFUL to their declared filters; the lie was in the charts. A filter change alters what a measure MEANS, which orphans every cell carrying it (the narrowed `generation_id` preserves redefinition-orphaning as correct) — a fleet rebuild to fix a label. Traffic/P95/Error Rate now state they cover inbound AND SDK-instrumented outbound calls. monoscope `48277bf51` | — |
| 28 | ✅ **DONE** — shipped with 15 | Error Rate was the only widget of ten on a 2-term scope, so it never matched the declared measure and scanned raw (~2 declines/hr). Third term added; it now routes on EXISTING measures. Held until 15 concluded, because shipping earlier would have spread the convention under review | — |
| 6 | ❌ **DECIDED: WILL NOT DO** | Measured demand is ONE chart and zero historical misses are level-shaped (#13). A DIMENSION change orphans the tier — no per-cell rescue exists, unlike measures. The repair machinery needed three fixes this week to work at all. The chart's cost is a raw scan already under attack from cheaper directions. **Reopening needs NEW evidence, not elapsed time** | — |
