...
| 4 — packed repairs on | `timefusion_rollup_packed_repairs` defaults to **true** (env `TIMEFUSION_ROLLUP_PACKED_REPAIRS=false` is the kill switch). Interrupted packed replacement: resume publishes once from the staged paths and queues no raw work. Restart recovery re-queues only slices ABOVE the repaired hour — the row witnesses (whole-date count, rows below a slice's end) do exactly that for unpacked slices too, so packing adds no restart work. datafusion-postgres pinned to `68fdc80` | Deployed `d1a226e0` (image `ce5ab58d`). Prod's pre-proof packed files then failed every packed repair *after* its scan ("lacks complete, consistent output evidence", ~10/15 min); fixed forward in `ee2ea352`/`61797239` (image `e1da854c`): the evidence check runs before the scan and unprovable publications escalate to the covering slice. 0 failures, 0 ERRORs since |

Rollup hit rate after the DataFusion 55 upgrade fell to ~0: the plan cache lifted `now()` as a NANOSECOND `TIMESTAMPTZ` placeholder, so coercion cast the microsecond `timestamp` column and CSE hoisted `CAST(timestamp AS Timestamp(ns))` into a projection the rollup matcher cannot walk — every `now()`-relative dashboard query declined `unwalkable_source` (reproduced over local pgwire). Fixed by binding the instant at microsecond precision (`TIMESTAMP(6) WITH TIME ZONE`). Measured on `e1da854c` at ~1h uptime before the fix: 0 hits/10 min vs 1530 misses/h, CPU 6–41 cores, 21–55 GB. `datafusion.execution.memory_fraction` is not a DataFusion option (startup warns, default kept) — pre-existing.

Old-process shutdown can panic in `buoyant_kernel_engine` executor (`TokioMultiThreadExecutor has crashed: RecvError`) after the WAL drained cleanly; harmless at handoff, not yet fixed.
