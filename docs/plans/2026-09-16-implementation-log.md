...
| Current write result: metrics coalesced commit | September 17 11:31 UTC; current production line, exact revision absent | One metrics group reported no commit result. The message alone does not establish acknowledged-data loss, but landing latency and replay state are unresolved. | `264cdb65` | TimeFusion write path. Trace the group through WAL, retry, and the Delta version; preserve it as unresolved until landed-row reconciliation proves recovery. |
