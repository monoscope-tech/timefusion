...
| --- | --- | --- |
| 01 | Serialization deployed; proxy-safe lease hotfix in review | Fingerprint `455e7ae0bbfa7d80ac7e1515f077270b`, Monoscope's original seven-day endpoint auto-ack evidence query, produced at least 50 failures in six hours. PR #572 replaced it with seven sequential daily slices, but the replacement fingerprint still timed out under concurrent scheduled work. Commit `8fe6f4cc` deployed at 17:21 UTC. Its first 17:30 cycle produced one completed sweep and ten `lease_busy` deferrals, proving serialization, then failed release because a session advisory lock cannot survive production transaction pooling. Monoscope PR #578 changes the same key to a transaction-scoped lock with a heartbeat. Keep item 01 open until that hotfix deploys and completes a clean cycle. |
