...
| --- | --- | --- | --- | --- |
| Current client SQL: endpoint dependency rollup | September 17 15:50 UTC; current production line, exact revision absent | The endpoint dependency rollup cannot run through TimeFusion, so the current caller loses its accelerated path. | `7b589527` (`OuterReferenceColumn` for correlated `UNNEST(e.hashes)`) | Monoscope owns the caller rewrite and TimeFusion owns the contract regression. Use scalar `UNNEST` in a derived projection, prove equal PostgreSQL and TimeFusion results, then deploy after the endpoint auto-ack serialization observation is complete. |
...
- Monoscope endpoint auto-ack serialization: PR #576 merged as `8fe6f4cc`. The canonical container build passed, the unit suite passed 325 examples, doctests passed 1,620 examples, and the focused real-Postgres/real-TimeFusion `autoAckProvenEndpoints` group passed five examples. GitHub then passed HLint, the clean x86 build, weeder, doctests, unit tests, the full integration suite, E2E, UI tests, formatting, and CodeQL. The external Claude reviewer failed before producing a review; it reported no engineering finding.
