...
| 07–08 | Transient object-store classes confirmed | The two named issues are transport failures: one S3 GET failed while sending the request, and one vacuum/list operation failed while reading the response body. Bounded September 11–17 searches found the same response-body class on September 11, 12, and 16, and request-send failures on September 13–15. No inspected event reports a missing object or authorization failure. Continue daily recurrence measurement; do not treat these transport failures as evidence of lost objects. |
