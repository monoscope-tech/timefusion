...
type Combine = fn(Box<Membership>, Box<Membership>) -> Membership;
...
fn strings(expr: &Expr) -> Option<Vec<String>> {
    ...
    (0..values.len()).map(|row| utf8(&ScalarValue::try_from_array(&values, row).ok()?).map(str::to_owned)).collect()
}
...
fn jsonpath_member(path: &str) -> Option<String> {
    // Must validate with the same parser as the UDF; the final serde_json parse is what preserves quotes/escapes/Unicode.
    ...
    match unalias(expr) {
        Expr::ScalarFunction(function) => match (function.name(), function.args.as_slice()) {
            ...
            (name @ ("array_has_all" | "array_has_any"), [Expr::Column(column), values]) => {
                let combine: Combine = if name == "array_has_all" { Membership::And } else { Membership::Or };
                let contains = |value| Membership::Contains { column: column.name.clone(), value };
                strings(values)?.into_iter().map(contains).reduce(|left, right| combine(Box::new(left), Box::new(right)))
            }
            _ => None,
        },
        Expr::BinaryExpr(binary) => {
            let combine: Combine = match binary.op {
                Operator::And => Membership::And,
                Operator::Or => Membership::Or,
                _ => return None,
            };
            Some(combine(Box::new(membership(&binary.left)?), Box::new(membership(&binary.right)?)))
        }
        _ => None,
    }
}
...
pub(crate) fn match_query(plan: &LogicalPlan) -> Option<HistogramQuery<'_>> {
    ...
    (count.params.args.len() <= 1 && count.params.args.first().is_none_or(|arg| matches!(arg, Expr::Literal(value, _) if !value.is_null()))).then_some(())?;
    ...
    // Only exactly adjacent ranges with the same source and predicate may be recombined.
    // UNION maps columns by position, so names must agree before walking the branches.
...
fn match_source<'a>(matched: &'a LogicalPlan, input: &LogicalPlan, width: i64) -> Option<HistogramQuery<'a>> {
    ...
    while let Some(expr) = pending.pop() {
        if let Expr::BinaryExpr(binary) = unalias(expr) {
            ...
            if binary.op == Operator::Eq && column(&binary.left, "project_id") {
                ...
                project.as_deref().is_none_or(|old| old == value).then_some(())?;
            ...
            if column(&binary.left, "timestamp") {
                ...
                let value = value.checked_add(i64::from(matches!(binary.op, Operator::Gt | Operator::LtEq)))?;
                match binary.op {
                    Operator::GtEq | Operator::Gt => lo = Some(lo.map_or(value, |old| old.max(value))),
                    Operator::Lt | Operator::LtEq => hi = Some(hi.map_or(value, |old| old.min(value))),
                    _ => return None,
                }
                continue;
            }
    ...
    membership.columns().iter().all(elements).then_some(())?;
