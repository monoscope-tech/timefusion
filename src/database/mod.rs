...
impl ProjectRoutingTable {
    ...
    async fn scan_delta_with_tantivy(
        ...
        query_time_range: Option<(i64, i64)>, bloom_rejected: Option<&HashSet<String>>,
        ...
        let Some(covered) = covered_files else {
            ...
            return Ok(vec![self.scan_delta_table(table, state, projection, &narrowed, limit, None, exclude, row_selections).await?]);
        };
        ...
        if raw.is_empty() && !indexed.is_empty() && !bloom_pruned_any {
            let narrowed = narrow(filters);
