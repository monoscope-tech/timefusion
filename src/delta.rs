...
impl CachedDeltaTableBuilder {
    ...
    pub fn with_storage_option<K: Into<String>, V: Into<String>>(
        mut self, 
        key: K, 
        value: V
    ) -> Self {
    ...
    async fn create_base_object_store(&self) -> Result<ObjectStoreRef, DeltaTableError> {
        let uri = Url::parse(&self.table_uri)
            .map_err(|e| DeltaTableError::Generic(format!("Invalid URI: {}", e)))?;
        ...
        match uri.scheme() {
            "s3" | "s3a" => {
                ...
                let bucket = uri.host_str()
                    .ok_or_else(|| DeltaTableError::Generic("No bucket in S3 URI".to_string()))?;
                
                Ok(Arc::new(builder.with_bucket_name(bucket).build()
                    .map_err(|e| DeltaTableError::ObjectStore { source: e })?))
            }
            ...
            "gs" => {
                ...
                let bucket = uri.host_str()
                    .ok_or_else(|| DeltaTableError::Generic("No bucket in GCS URI".to_string()))?;
            ...
            "file" => {
                let path = uri.to_file_path()
                    .map_err(|_| DeltaTableError::Generic("Invalid file path".to_string()))?;
...
mod tests {
    ...
    async fn test_cached_delta_table_creation() {
        ...
        let table = CachedDeltaOps::create_table(
            &table_uri,
            schema.clone(),
            Some(cache_config),
        ).await.unwrap();
    ...
    async fn test_write_and_read_with_cache() {
        ...
        let mut table = CachedDeltaOps::create_table(
            &table_uri,
            schema.clone(),
            Some(DeltaCacheConfig::default()),
        ).await.unwrap();
