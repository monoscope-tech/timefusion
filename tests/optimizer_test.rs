...
use timefusion::optimizers::{ProjectIdPushdown, time_range_partition_pruner};
...
fn test_project_id_filter_detection() {
    ...
    let combined_filter = Expr::BinaryExpr(BinaryExpr::new(Box::new(project_filter), Operator::And, Box::new(other_filter)));
