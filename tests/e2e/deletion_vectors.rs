//! Merge-on-read deletion vectors end-to-end: with `timefusion_use_deletion_vectors`
//! on, a Delta UPDATE/DELETE masks matched rows with a roaring-bitmap deletion vector
//! instead of rewriting whole files.
//!
//! Subject is `mor_dormant`, not otel: `otel_logs_and_spans` sets `version_append`, so
//! an UPDATE there appends a row version instead of masking and cannot witness DVs.
...
/// Seed `n` rows and flush into exactly ONE Delta parquet file, so DML targets Delta,
/// not MemBuffer. Advancing past the bucket duration is what makes the bucket flushable.
...
/// Maintenance dedup drops a cross-file physical duplicate by masking the loser with a
/// deletion vector, not by rewriting files; OPTIMIZE then consolidates the DV without
/// resurrecting the row. Uses a PAST partition so `dedup_partition` (slice=None) clears
/// the 2h sealed-chunk guard, which the future-dated `FROZEN_START_MICROS` never would.
#[serial_test::serial]
#[tokio::test(flavor = "multi_thread")]
async fn dv_dedup_drops_cross_file_duplicate_without_rewriting() -> anyhow::Result<()> {
    use std::collections::HashSet;

    // Long flush interval: the two explicit flushes must define the two source files,
    // or a background tick between INSERTs splits the fixture.
    ...
    // OPTIMIZE reads DV-masked, drops the loser physically, writes DV-free files.
...
/// Deletion vectors live in the Delta log, not the WAL: a restart must reload them from
/// the committed log, so the snapshot-reload path must not drop DV descriptors.
#[serial_test::serial]
#[tokio::test(flavor = "multi_thread")]
...
/// OPTIMIZE must consolidate deletion vectors into DV-free files without resurrecting
...
/// `UPDATE ... FROM` (the MERGE shape) as merge-on-read: matched target rows are masked
/// and their updated copies appended, not whole-file rewritten.
#[serial_test::serial]
#[tokio::test(flavor = "multi_thread")]
async fn dv_merge_update_from_source_masks_and_appends() -> anyhow::Result<()> {
    ...
    // Routes through perform_delta_merge_update -> DV merge op.
    client
        .execute(
