...
mod integration {
    ...
    async fn test_concurrent_postgres_requests() -> Result<()> {
        ...
        for i in 0..num_clients {
            ...
            let handle = tokio::spawn(async move {
                ...
                for j in 0..ops_per_client {
                    ...
                    if j % 3 == 0 {
                        let _query_result = client.query(
                            "SELECT COUNT(*) FROM otel_logs_and_spans WHERE project_id = $1", 
                            &[&"test_project"]
        ...
        let count_rows = client.query(
            &format!("SELECT COUNT(*) FROM otel_logs_and_spans WHERE id LIKE '{test_id}%'"), 
            &[]
        ...
        let id_rows = client.query(
            &format!("SELECT id FROM otel_logs_and_spans WHERE id LIKE '{test_id}%'"), 
            &[]
        ).await.map_err(|e| anyhow::anyhow!("Query failed: {}", e))?;
                ...
                for j in 0..queries_per_client {
                    ...
                    match j % 3 {
                        ...
                        _ => {
                            // Aggregate query
                            let _ = client.query(
                                "SELECT status_code, COUNT(*) FROM otel_logs_and_spans GROUP BY status_code", 
                                &[]
                            ).await.expect("Query should succeed");
                        }
                    }
